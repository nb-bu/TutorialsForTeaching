...
To illustrate, the following table shows the choices of 10 participants (columns) on the first 10 choice problems (rows). Here, values of 0 refer to a choice of `Option A`, and values of 1 mean that `Option B` was chosen. 
...
Since we know for all problems whether `Option A` (`0`) or `Option B` (`1`) has the higher EV, we can compute the proportion of choices, in which participants chose the option with the higher EV. 
...
So far, CPT can only make deterministic predictions (e.g., always choose `Option A` when $V_A > V_B$). Assuming a probabilistic world, where people mostly choose according to their overall preference, but may sometimes also act differently depending on context (e.g., peer pressure or other factors), we need to make one more adjustment to CPT to make it probabilistic instead of deterministic. Doing so, CPT will be able to provide us with probabilities that can also be different from 0 and 1.
...
If their evaluations are similar, the choice probability gets closer to $.5$, indicating that a person might be indifferent between the two choice options.
...
setwd(paste(pathname, "/project_data",sep=""))

# import choice data
desc_choices_kellen2016 <- openxlsx::read.xlsx(here("tutorials", "BayesianCognitiveModeling", "kellen2016.xlsx"), sheet = "Choices_description")  %>% dplyr::rename(label = 1)
choices_long = desc_choices_kellen2016 %>% 
  gather(key = "code", value = "choice_num", -label) # gather choice data


# read in gambles
gambles.all <- openxlsx::read.xlsx(here("tutorials", "BayesianCognitiveModeling", "kellen2016.xlsx"),sheet = "Lottery_problems") %>% 
  dplyr::rename(probA3 = 7, outcomeA3 = 8, probB3 = 13, outcomeB3 = 14 # add missing variable names for third outcomes
         , ProblemID = problem) # rename problem variables
gambles.all[is.na(gambles.all)==T] <- 0
gambles = gambles.all %>% select(ProblemID, label,
                                 outcomeA1, probA1, outcomeA2, probA2, #outcomeA3, probA3, # remove dummy outcome 3 - because we add one again for all datasets later
                                 outcomeB1, probB1, outcomeB2, probB2, #outcomeB3, probB3
                                 ) # need to order it with outcome first, for ordering by size command

desc_kellen2016 = merge(gambles, choices_long, by = "label") 


# Cprepare data for formatting loop -> renaming variables
desc_kellen2016 <- desc_kellen2016  %>% select(!label) %>% dplyr::rename(A1o = outcomeA1, A1p = probA1,
                                                                         A2o = outcomeA2, A2p = probA2,
                                                                         B1o = outcomeB1, B1p = probB1,
                                                                         B2o = outcomeB2, B2p = probB2) # rename variables


## Experience

# read in choices
exp_choices_kellen2016 <- openxlsx::read.xlsx(here("tutorials", "BayesianCognitiveModeling", "kellen2016.xlsx"), sheet = "Choices_experience_neu") %>% dplyr::rename(ProblemID = 1)
choices_long = exp_choices_kellen2016 %>% 
  gather(key = "ParticipantCode", value = "choice_num", -ProblemID) # gather choice data

# read in gambles
gambles.all_probsexp <- openxlsx::read.xlsx("kellen2016.xlsx", sheet = "Lottery_problems_experienced") %>% # individually experienced probabilities from samples
  dplyr::rename(probA3 = 8, outcomeA3 = 9, probB3 = 14, outcomeB3 = 15) # add missing variable names for third outcomes
gambles.all_probsexp[is.na(gambles.all_probsexp)==T] <- 0

gambles = gambles.all_probsexp %>% 
  select(ProblemID, ProblemName, ParticipantCode,
                                 outcomeA1, probA1, outcomeA2, probA2, #outcomeA3, probA3, # remove dummy outcome 3
                                 outcomeB1, probB1, outcomeB2, probB2, #outcomeB3, probB3
                                 ) # need to order it with outcome first, for ordering by size command

exp_kellen2016 = merge(gambles, choices_long, by = c("ParticipantCode", "ProblemID")) 
exp_kellen2016 = exp_kellen2016 %>% 
  select(!ProblemID) %>%
  ...
  arrange(code, ProblemID)

# read in sampling info
samples_kellen<- openxlsx::read.xlsx("kellen2016.xlsx", sheet = "Sampling_experience") #%>% # individually experienced probabilities from samples
 # dplyr::rename(probA3 = 8, outcomeA3 = 9, probB3 = 14, outcomeB3 = 15) # add missing variable names for third outcomes
# gambles.all_probsexp[is.na(gambles.all_probsexp)==T] <- 0
...
samples_kellen<- samples_kellen %>%
  
  mutate(
    choice_num = case_when(
      response == "A" ~ 1,
      response == "B" ~ 0,  # new variable with only numerical choice
      TRUE ~ NA)  ,

    sampled_option = case_when(
      SampVal == "A1" | SampVal == "A2" ~ "A",
      SampVal == "B1" | SampVal == "B2" ~ "B",
      TRUE ~ NA) ,# new variable with sampled option
    
    NSA = case_when(
      SampVal == "A1" | SampVal == "A2" ~ 1,
      SampVal == "B1" | SampVal == "B2" ~ 0,
      TRUE ~ NA) ,# new variable with sampled option A count
    
    NSB = case_when(
      SampVal == "A1" | SampVal == "A2" ~ 0,
      SampVal == "B1" | SampVal == "B2" ~ 1,
      TRUE ~ NA) ,# new variable with sampled option B count
        
    probA1_exp = case_when(
      SampVal == "A1"  ~ 1,
      SampVal == "A2"  ~ 0,
      TRUE ~ NA) , # new variable with probability of outcome A1
    
    probB1_exp = case_when(
      SampVal == "B1"  ~ 1,
      SampVal == "B2"  ~ 0,
      TRUE ~ NA) # new variable with probability of outcome B1
  )
samples_kellen <- samples_kellen %>%
  dplyr::group_by(ParticipantCode, ProblemID, ProblemName) %>%
  dplyr::summarise(
    outcomeA1 = mean(outcomeA1, na.rm = TRUE), # retain gamble descriptions (ground truth outcomes and probs)
    probA1 = mean(probA1, na.rm = TRUE),
    outcomeA2 = mean(outcomeA2, na.rm = TRUE),
    probA2 = mean(probA2, na.rm = TRUE),
    outcomeB1 = mean(outcomeB1, na.rm = TRUE),
    probB1 = mean(probB1, na.rm = TRUE),
    outcomeB2 = mean(outcomeB2, na.rm = TRUE),
    probB2 = mean(probB2, na.rm = TRUE),
    choice_num = mean(choice_num, na.rm = TRUE),
    NSA = sum(NSA, na.rm = TRUE), # sum up sampled outcomes
    NSB = sum(NSB, na.rm = TRUE),
    probA1_exp = mean(probA1_exp, na.rm = TRUE), # average actual experienced probabilities of outcomes
    probB1_exp = mean(probB1_exp, na.rm = TRUE)
  ) %>%
  mutate(probA2_exp = 1-probA1_exp, # get experienced probability of contrasting outcome for each option
         probB2_exp = 1-probB1_exp,
         samples = NSA + NSB)       # get total number of samples per trial

samples_kellen = samples_kellen %>% mutate(experiment = studies[s]) %>% select(experiment, code = ParticipantCode, ProblemID, samples)
samples_drawn = rbind(samples_drawn, samples_kellen)
